    ...
    where TRequest : VentaRequest
{
    protected VentaRequestValidator()
    {
        ...
        RuleFor(x => x.Detalles)
            ...
            .WithMessage(
                "No pueden haber servicios duplicados en los detalles de venta.");

        RuleForEach(x => x.Detalles)
            .SetValidator(
                new VentaDetalleRequestValidator<VentaDetalleRequest>());
        ...
        RuleForEach(x => x.Pagadores)
            .SetValidator(
                new VentaPagadorRequestValidator<VentaPagadorRequest>());
    }
}
    ...
    where TRequest : VentaDetalleRequest
{
    public VentaDetalleRequestValidator()
    {
        ...
        RuleFor(x => x.Descuento)
            ...
            .WithMessage(
                "El descuento no puede superar el importe del detalle.");

        RuleFor(x => x.MedicoId)
            .GreaterThan(0)
            .WithMessage(
                "El médico debe ser válido.")
    ...
    where TRequest : VentaPagadorRequest
{
    public VentaPagadorRequestValidator()
    {
        ...
        RuleFor(x => x.ConvenioId)
            .Empty()
            .WithMessage(
                "Un pagador de paciente no puede tener convenio.")
            .When(x =>
        ...
        RuleFor(x => x.Monto)
            .GreaterThan(0)
            .WithMessage(
                "El monto del pagador debe ser mayor a cero.");
    }
}
