using Clinica.Api.Modules.Cajas.AperturaCaja.Entity;
using Clinica.Api.Shared.Persistence;
using Microsoft.EntityFrameworkCore;
using Microsoft.EntityFrameworkCore.Metadata.Builders;

namespace Clinica.Api.Data.Configurations;

public sealed class AperturaCajaConfiguration
    : AuditableEntityConfiguration<AperturaCaja>
{
    protected override void ConfigureEntity(
        EntityTypeBuilder<AperturaCaja> builder)
    {
        builder.ToTable("AperturasCaja");

        builder.Property(x => x.TurnoCajaId)
            .IsRequired();

        builder.Property(x => x.FechaHora)
            .IsRequired();

        builder.Property(x => x.MontoInicial)
            .HasPrecision(18, 2)
            .IsRequired();

        builder.Property(x => x.Observacion)
            .HasMaxLength(500);

        builder.HasIndex(x => x.TurnoCajaId);

        builder.HasOne(x => x.TurnoCaja)
            .WithMany()
            .HasForeignKey(x => x.TurnoCajaId)
            .OnDelete(DeleteBehavior.Restrict);
    }
}