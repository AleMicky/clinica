...
interface CargoPageFormProps {
  id?: number;
}

export function CargoPageForm({ id }: CargoPageFormProps) {
  const router = useRouter();
  const isEditing = Boolean(id && id > 0);

  const { data: cargoData, isLoading: isLoadingCargo } = useCargo(
    id || 0,
    isEditing
  );
  ...
  // Load existing cargo data in edit mode
  React.useEffect(() => {
    if (cargoData && isEditing) {
      reset({
  ...
  const onSubmit = async (values: CargoFormValues) => {
    try {
      ...
      router.push("/recursos-humanos/cargos");
    } catch (error: any) {
      const errorMsg =
      ...
      <div className="flex flex-col sm:flex-row sm:items-center justify-between gap-3 pb-3 border-b border-border/60">
            ...
            <div className="min-w-0">
              <div className="flex items-center gap-2">
                <h1 className="text-sm sm:text-base font-bold text-foreground truncate">
                  {isEditing
                    ? `Editar Cargo: ${cargoData?.nombre || ""} (#${cargoData?.codigo || ""})`
              ...
              <p className="text-[11px] text-muted-foreground truncate">
                {isEditing
                  ? "Actualice los parámetros de este cargo o posición laboral."
                  : "Defina un nuevo cargo para asignación de funciones a empleados."}
              </p>
            </div>
          </div>
        </div>
        ...
        <div className="flex items-center gap-2 self-end sm:self-auto shrink-0">
          ...
          >
            ...
            <span>{isEditing ? "Guardar Cambios" : "Registrar Cargo"}</span>
          </Button>
        </div>
      </div>
            ...
            <div className="space-y-3">
              <div className="flex items-center gap-2 pb-2 border-b border-border/60">
                ...
                <div>
                  ...
                  <p className="text-[11px] text-muted-foreground">
                    Código de identificación único y denominación oficial del puesto.
                  </p>
                </div>
              </div>
              ...
              <div className="grid grid-cols-1 sm:grid-cols-3 gap-3">
                ...
                <div className="space-y-1 sm:col-span-2">
                  ...
                  <Input
                    id="nombre"
                    placeholder="ej. Enfermero(a) Jefe de Guardia"
            ...
            <div className="space-y-3 pt-2">
              <div className="flex items-center gap-2 pb-2 border-b border-border/60">
                ...
                <div>
                  ...
                  <p className="text-[11px] text-muted-foreground">
                    Detalle complementario de responsabilidades y alcance del puesto.
                  </p>
                </div>
              </div>
              ...
              <div className="space-y-1">
                ...
                <Textarea
                  id="descripcion"
                  placeholder="Detalle breve de las actividades y alcance del cargo..."
