...
interface BajaInventarioHeaderProps {
  totalItems?: number;
  onRefresh?: () => void;
  isRefreshing?: boolean;
}

export function BajaInventarioHeader({
  totalItems = 0,
  onRefresh,
  isRefreshing = false,
}: BajaInventarioHeaderProps) {
  return (
      ...
      <div className="flex items-center gap-2.5">
        ...
        <div>
          <div className="flex items-center gap-2">
            ...
            <Badge
              variant="outline"
              className="text-[10px] h-4.5 px-1.5 font-semibold text-rose-600 dark:text-rose-400 border-rose-500/30 bg-rose-500/5"
            >
              {totalItems} {totalItems === 1 ? "registro" : "registros"}
            </Badge>
          </div>
          <p className="text-[11px] text-muted-foreground leading-tight">
            Gestión y descarte formal de existencias por vencimiento, daño, rotura o mermas clínicas
          </p>
        </div>
      </div>
