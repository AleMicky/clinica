...
interface InventarioFisicoHeaderProps {
  totalItems?: number;
  onRefresh?: () => void;
  isRefreshing?: boolean;
}

export function InventarioFisicoHeader({
  totalItems = 0,
  onRefresh,
  isRefreshing = false,
}: InventarioFisicoHeaderProps) {
  return (
        ...
        <div>
          <div className="flex items-center gap-2">
            <h1 className="text-sm sm:text-base font-bold tracking-tight text-foreground">
              Inventarios Físicos y Arqueos de Stock
            </h1>
            <Badge
              variant="outline"
          ...
          <p className="text-[11px] text-muted-foreground leading-tight">
            Control de conteos físicos, detección de mermas/sobrantes y ajustes automáticos de stock
          </p>
        </div>
      </div>
