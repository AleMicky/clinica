...
export function TransferenciaAlmacenFormDialog({
  ...
  const watchedOrigenId = watch("almacenOrigenId");
  const watchedDestinoId = watch("almacenDestinoId");
  ...
  const onSubmit = async (values: TransferenciaAlmacenFormValues) => {
    ...
    try {
      if (isEditing && transferenciaToEdit) {
        ...
        toast.success(`Transferencia "${payload.numero}" actualizada.`);
      } else {
      ...
      <DialogContent className="sm:max-w-4xl max-h-[92vh] flex flex-col p-6 overflow-hidden">
        ...
        <DialogHeader className="pb-3 border-b border-border/40 shrink-0">
          <div className="flex items-center justify-between">
            <div className="flex items-center gap-3">
              ...
              <div>
                ...
                <DialogDescription className="text-xs text-muted-foreground">
                  {isEditing
                    ? "Modifica los datos del borrador de solicitud de transferencia"
                    : "Registra una solicitud de traspaso de productos entre depósitos"}
                </DialogDescription>
              </div>
            </div>
            ...
            <Badge
              variant="outline"
              className="text-[11px] h-6 px-2.5 font-semibold text-amber-600 border-amber-500/30 bg-amber-500/10"
            >
              Borrador
            </Badge>
          </div>
          ...
          <div className="p-3.5 rounded-xl bg-muted/30 border border-border/60 flex flex-col gap-3.5 shadow-2xs">
            <div className="grid grid-cols-1 sm:grid-cols-3 gap-3">
              ...
              <div className="flex flex-col gap-1">
                <Label htmlFor="numero" className="text-xs font-medium">
                  N° Transferencia <span className="text-destructive">*</span>
                </Label>
                <Input
                  id="numero"
                  {...register("numero")}
            ...
            <div className="flex flex-col gap-1">
              ...
              <Textarea
                ...
                placeholder="Motivo del traslado, proyecto de destino, urgencia..."
                rows={2}
              ...
              <table className="w-full text-left text-xs border-collapse">
                <thead>
                  <tr className="border-b border-border/60 bg-muted/40 text-muted-foreground font-semibold">
                    ...
                    <th className="px-3 py-2 w-32">Cant. Solicitada *</th>
