...
interface MovimientoInventarioListProps {
  ...
  selectedTipoMovimientoId?: number | null;
  selectedEstado?: EstadoMovimientoInventario | null;
  onSearchChange?: (value: string) => void;
  onAlmacenChange?: (almacenId: number | null) => void;
  onTipoMovimientoChange?: (tipoId: number | null) => void;
  onEstadoChange?: (estado: EstadoMovimientoInventario | null) => void;
  onPageChange?: (page: number) => void;
  onPageSizeChange?: (size: number) => void;
  onAddMovimiento?: () => void;
  onViewDetail?: (movimiento: MovimientoInventarioResponse) => void;
  onEdit?: (movimiento: MovimientoInventarioResponse) => void;
  onConfirm?: (movimiento: MovimientoInventarioResponse) => void;
  onAnular?: (movimiento: MovimientoInventarioResponse) => void;
  onDelete?: (movimiento: MovimientoInventarioResponse) => void;
  onRefresh?: () => void;
  onViewAudit?: (movimiento: MovimientoInventarioResponse) => void;
}
...
function getEstadoBadge(estado: EstadoMovimientoInventario) {
  switch (estado) {
...
export function MovimientoInventarioList({
  movimientos,
  isLoading = false,
  totalItems = 0,
  currentPage = 1,
  ...
  const handleTabChange = (val: string) => {
    if (val === "all") onEstadoChange?.(null);
      ...
      onEstadoChange?.(EstadoMovimientoInventario.Borrador);
    else if (val === "confirmado")
      onEstadoChange?.(EstadoMovimientoInventario.Confirmado);
    else if (val === "anulado")
      onEstadoChange?.(EstadoMovimientoInventario.Anulado);
  };

  const currentTab =
    selectedEstado === null
      ? "all"
      : selectedEstado === EstadoMovimientoInventario.Borrador
      ? "borrador"
      : selectedEstado === EstadoMovimientoInventario.Confirmado
      ? "confirmado"
      : "anulado";
  ...
  const handleExportCsv = () => {
    ...
    const rows = movimientos.map((m) => [
      m.id,
      `"${m.numero.replace(/"/g, '""')}"`,
      `"${(m.tipoMovimientoNombre || "").replace(/"/g, '""')}"`,
      `"${(m.almacenNombre || "").replace(/"/g, '""')}"`,
      `"${m.fechaMovimiento}"`,
      m.estado === EstadoMovimientoInventario.Borrador
        ? "Borrador"
        : m.estado === EstadoMovimientoInventario.Confirmado
        ? "Confirmado"
        : "Anulado",
      `"${(m.referenciaTipo || "").replace(/"/g, '""')}"`,
      m.referenciaId || "",
  ...
  return (
    <div className="flex flex-col gap-2.5 bg-card border border-border/60 rounded-lg p-3 shadow-2xs">
      ...
      {/* Filter Toolbar with Autocompletes */}
      <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-4 gap-2 items-center">
        ...
        <form onSubmit={handleSearchSubmit} className="relative w-full">
          ...
          <Input
            ...
            placeholder="Buscar por número, ref u obs..."
        ...
        {/* Tipo Movimiento Autocomplete Filter */}
        <div className="w-full">
          <TipoMovimientoInventarioAutocomplete
            value={selectedTipoMovimientoId}
            onValueChange={(val) => onTipoMovimientoChange?.(val)}
            placeholder="Filtrar por tipo movimiento..."
          />
        </div>
                  ...
                  >
                    ...
                    <td className="px-2.5 py-1.5">
                      <div className="flex items-center gap-1 text-muted-foreground text-xs">
                        ...
                        <span className="truncate">{mov.almacenNombre || "-"}</span>
                      </div>
                    </td>
                    <td className="px-2.5 py-1.5 text-muted-foreground font-mono text-[10px]">
                      {formatDate(mov.fechaMovimiento)}
                    </td>
                    <td className="px-2.5 py-1.5">{getEstadoBadge(mov.estado)}</td>
                    ...
                    >
                      <DropdownMenu>
                        <DropdownMenuTrigger
                          ...
                          aria-label={`Acciones de ${mov.numero}`}
                        >
