...
import {
  ...
  type AnularDevolucionFormValues,
} from "../schemas/devolucion-proveedor.schema";
import { useAnularDevolucionProveedor } from "../hooks/use-devolucion-proveedor";
import type { DevolucionProveedorResponse } from "../types/devolucion-proveedor.types";

interface DevolucionProveedorAnularDialogProps {
  open: boolean;
...
export function DevolucionProveedorAnularDialog({
  open,
  onOpenChange,
  devolucion,
  onSuccessCallback,
}: DevolucionProveedorAnularDialogProps) {
  const anularMutation = useAnularDevolucionProveedor();
  ...
  } = useForm<AnularDevolucionFormValues>({
    resolver: zodResolver(anularDevolucionSchema),
    defaultValues: {
      motivoAnulacion: "",
    },
  });
  ...
  const onSubmit = async (values: AnularDevolucionFormValues) => {
    ...
    try {
      ...
      toast.success(
        `Devolución "${devolucion.numero}" anulada y stock reintegrado.`
      );
      onSuccessCallback?.();
      onOpenChange(false);
    } catch (error: any) {
      ...
      <DialogContent className="sm:max-w-md p-5">
          ...
          <div className="flex items-center gap-2.5">
            ...
            <div>
              <DialogTitle className="text-sm font-bold text-foreground">
                Anular Devolución a Proveedor
              </DialogTitle>
              <DialogDescription className="text-xs text-muted-foreground pt-0.5">
                Esta acción revertirá la salida de stock al inventario
              </DialogDescription>
            </div>
          </div>
        </DialogHeader>
        ...
        <form onSubmit={handleSubmit(onSubmit)} className="space-y-3 py-2 text-xs">
          <p className="text-muted-foreground">
            Indica el motivo por el cual se anula la devolución{" "}
          ...
          <div className="flex flex-col gap-1">
            ...
            <Textarea
              ...
              placeholder="Ej. Rechazo del proveedor a recibir, acuerdo de nota de débito..."
              rows={3}
