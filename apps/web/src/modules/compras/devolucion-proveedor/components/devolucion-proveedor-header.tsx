...
interface DevolucionProveedorHeaderProps {
  totalItems?: number;
  onRefresh?: () => void;
  isRefreshing?: boolean;
}

export function DevolucionProveedorHeader({
  totalItems = 0,
  onRefresh,
  isRefreshing = false,
}: DevolucionProveedorHeaderProps) {
  return (
      ...
      <div className="flex items-center gap-2.5">
        ...
        <div>
          <div className="flex items-center gap-2">
            ...
            <Badge
              variant="outline"
              className="text-[10px] h-4.5 px-1.5 font-semibold text-orange-600 dark:text-orange-400 border-orange-500/30 bg-orange-500/5"
            >
              {totalItems} {totalItems === 1 ? "registro" : "registros"}
            </Badge>
          </div>
          <p className="text-[11px] text-muted-foreground leading-tight">
            Gestión y salida de existencias devueltas por fallas, daños, garantías o vencimiento
          </p>
        </div>
      </div>
